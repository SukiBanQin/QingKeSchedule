...
这份文档供 MacBook 上新打开本仓库的 Codex 使用。目标是在不破坏现有 Web MVP 的前提下，按照 iOS 产品设计与技术方案实现原生 iPhone MVP，并建立可重复的 Xcode 构建、测试和免费 Personal Team 真机安装流程。
...
4. `src/domain/types.ts`：Web 数据字段和枚举。
5. `src/domain/rules.ts`：Web 核心业务规则。
6. `src/domain/validation.ts`：Web 校验行为。
7. `src/storage/localStorageRepository.ts`：`schemaVersion: 1` 格式。
8. `tests/unit/rules.test.ts` 与 `tests/unit/storage.test.ts`：已验证行为。
9. `docs/product-design.md` 与 `docs/technical-solution.md`：Web 背景与架构边界。
...
- Web MVP 已在提交 `0b97a6d` 中实现。
- Web 使用 Vue 3、TypeScript、Vite 和 localStorage。
- Web 当前数据版本为 `schemaVersion: 1`。
- iOS 产品设计已提交。
- Web 尚未实现与 iOS 对接所需的数据文件导出。
- GitHub 共享仓库为 `https://github.com/SukiBanQin/School_timetable.git`，Windows 端完成本次文档提交后应将它配置为 `origin` 并推送完整历史。
- 该仓库当前由用户创建为 Public；只提交源代码、测试和设计文档，不提交个人课表导出文件、Apple 凭据或设备信息。
...
- 不修改 Web 现有行为，除非任务明确要求实现跨端导出或共享契约。
- 所有日期、单双周和冲突行为与 Web 保持一致。
...
- 添加最小的 `scripts/ios-build.sh` 和 `scripts/ios-test.sh`。
- 提交一个只包含脚手架、脚本和测试的 commit。

### 阶段二：共享协议与领域规则

- 从现有 TypeScript 类型提取 `shared/schedule-data.schema.json`。
- 建立 Web 与 iOS 共用的 JSON fixtures。
- Swift DTO 保持 `schemaVersion: 1` 字段完全兼容。
- 用 Swift 纯函数实现教学周、单双周、今日筛选、状态和冲突检测。
- 让 Swift Testing 和现有 Vitest 对相同 fixtures 得出一致结论。
- 单独提交。
...
- 首次手动 Automatic Signing 成功后添加 `scripts/ios-install.sh`。
- 记录免费描述文件到期后的重新构建步骤。
...
Windows 与 Mac 通过同一个 GitHub 仓库同步，不直接复制工作目录：

```text
Windows：Web 开发、文档、共享协议
                    ↕ push / pull
GitHub 仓库：唯一共享提交历史
                    ↕ push / pull
...
```text
origin  https://github.com/SukiBanQin/School_timetable.git
```
...
在 GitHub 仓库克隆完成并用 MacBook Codex 打开根目录后，用户可以发送：

```text
请先完整阅读 AGENTS.md、docs/ios-product-design.md、docs/ios-technical-solution.md 和 docs/macbook-codex-handoff.md，再检查现有 Web 数据模型与测试。先只完成接续说明中的“阶段一：Xcode 工程与构建循环”，不要提前实现后续业务功能。使用当前稳定版 Xcode，自动发现可用的 iPhone Simulator，确保 build 和 test 通过，并按仓库要求创建独立 Git commit。不要修改或覆盖无关的 Web 代码，也不要提交任何 Apple Account、Team ID、证书或设备信息。
```
