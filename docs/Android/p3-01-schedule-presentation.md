...
后续基准更新（2026-09-13）：当前 `origin/IOS` 已通过 `c3191ae` 将课程状态和进度改为秒级，结束
时刻立即变为 `FINISHED`，并增加前台每秒与手动刷新。P3-01 原分析中的“结束分钟仍进行中”和整分钟
进度已不再是当前跨端基准；Android 将在 [P3-03 今日页任务](p3-03-today-schedule.md)中同步这一聚焦差异，
其余排序、下一门、教学日历、冲突和矩阵契约不变。
...
- iOS 基准以本地日历日期计算教学周，今日课程按开始节次、结束节次、`zh_CN` 名称和原始稳定顺序排列；
  当前秒级状态从开始时刻起进行中、到结束时刻立即结束，只标记第一门未来课程为“下一门”。
...
- 扩展 `ScheduleRules`（或等价纯 Kotlin 边界）提供：指定周和星期对应日期、某教学周全部发生项、
  课程状态及时间解析。课程状态为 `FINISHED`、`ONGOING`、`UPCOMING`；当前 iOS 基准以秒判断，开始
  时刻包含在 `ONGOING`，结束时刻包含在 `FINISHED`。学期、节次经现有验证器保证合法，不改变
  `ScheduleValidator` 的协议规则。
...
- 对排序后的课程计算状态；只把第一项 `UPCOMING` 标为下一门。仅 `ONGOING` 项提供进度：从首节开始
  到末节结束计算已过秒数、剩余秒数、派生分钟／倒计时文字和限制在 `0.0..1.0` 的比例。
...
- 覆盖学期前、学期末后一周、学期内无课；状态开始前、开始时刻、结束前一秒、结束时刻和结束后边界；无对应节次时
  的安全退化。
