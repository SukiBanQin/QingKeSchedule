...
**本机 API 37 ARM64 运行证据已补齐并通过独立专项复审；P1-03 授权范围完成。P1 阶段
关闭与用户验收不在本次结论内，不进入 P2。**
...
- 之后用 `adb emu kill` 正常关闭，再用同一稳定版带窗口启动，约 27 秒再次开机成功，
  APK 无需重装，MainActivity 冷启动成功（3325 ms）。执行交付时曾保留可见窗口；独立
  复审开始时该设备已不在线，复审另用端口 5588 完成聚焦验证后正常关闭。
...
与 APK 摘要关联。P1-03-R4 环境修复与运行证据已通过独立专项复审；本窗口没有实施应用代码。
不将本次最小启动扩展为 A01—A11、存储、通知、内存压力或完整依赖兼容测试通过。
未知字段已决定为两端严格拒绝，iOS 修正纳入 P1-04；不自动安排 P2。

独立复审还在当前 `target=android-37` 登记下以端口 5588 重现正常开机、安装和冷启动，
并用临时只读 `target=android-0` 登记副本重现 API 3／HVF 关闭／`mprotect` 失败；同时将
Gradle 的 70 个任务以 `--rerun-tasks` 实际执行，Debug／Release 各 20 项测试通过。详见
[R4 专项复审](p1-03-review.md)和
[R4 独立证据](evidence/p1-03-r4-review-20260908.txt)。
