...
最新状态（2026-09-19）：P3-05／A03 周课表已实现、测试、通过 Sol 技术独立复审并经用户确认视觉通过（实现提交 `e3321bd`，验收收口 `5fb0bd2`），A03 当前范围的验收门槛关闭。P3-06／A06 的 R6 已完成 Sol 技术复审并获用户视觉验收；用户随后授权 P3-06-R7，把删除节次影响已有课程的阻止保存改为一次性破坏性汇总确认与原子级联处理，该修正尚未实施。P3-07／A07 已实现、测试并通过 [Sol 独立技术复审](p3-07-review.md)，用户已确认视觉风格通过；实际使用发现无午休后续节次时午休条错误隐藏，产品规则同时确认为「午休与节次重叠时节次优先、但确认前弹一次红色警告，无冲突时无论每日节数多少都必须显示在矩阵顶部／间隙／底部」；P3-07-R1 已实施并自测（见 [R1 证据](evidence/p3-07-r1-lunch-break/README.md)），等待 Sol 独立复审与用户验收，因此 A07 功能验收仍重新打开。A08／A10／A11、整个 P3 与完整 App 仍未完成且未授权。
...
A06 R6 已通过技术复审与用户视觉验收但 R7 待实施；A07 原实现通过技术复审且视觉风格通过，R1 已实施并自测但复审与验收待进行，
...
| A07 | 周末停课、指定日期停课、调课日按指定星期上课，午休设置 | [教学日历设置](../../ios/QingKeSchedule/Notifications/ReminderSettings.swift) | 停课与调课优先级、单双周组合、今日／周表／通知一致；已实现、测试并通过 Sol 技术复审：Android 在首次设置与正式设置共享「03 教学日历」面板，写入立即持久化并驱动今日、周表，视觉风格已获用户确认；P3-07-R1 已实施并自测：午休与节次重叠时节次优先且确认前弹一次红色警告（返回修改／仍然保存），无冲突时无论每日节数多少都显示在矩阵顶部、对应间隙或底部，不修改课程、节次与停课规则，R1 复审与验收待进行；通知部分属 A08 未实施 |
