...
最新状态（2026-09-19）：P3-05／A03 周课表已实现、测试、通过 Sol 技术独立复审并经用户确认视觉通过（实现提交 `e3321bd`，验收收口 `5fb0bd2`），A03 当前范围的验收门槛关闭。P3-06／A06 的 R6 已完成 Sol 技术复审并获用户视觉验收；用户随后授权 P3-06-R7，把删除节次影响已有课程的阻止保存改为一次性破坏性汇总确认与原子级联处理，该修正尚未实施。P3-07／A07 原实现已通过技术复审，用户已确认视觉风格；P3-07-R1 已实施并自测，[首轮独立复审](p3-07-r1-review.md)发现两项缺口（首次设置／未保存节次草稿没有可靠触发冲突确认；「仍然保存」写入失败后界面会保留未写入的候选时间），两项修正已实施并自测（JVM 132／设备 110、7 张截图证据），等待 Sol 再复审与用户验收，A07 功能验收保持打开。A08／A10／A11、整个 P3 与完整 App 仍未完成且未授权。
...
A06 R6 已通过技术复审与用户视觉验收但 R7 待实施；A07 原实现通过技术复审且视觉风格通过，R1 首轮复审未通过、两项修正已实施并自测、再复审与验收待进行，
...
| A07 | 周末停课、指定日期停课、调课日按指定星期上课，午休设置 | [教学日历设置](../../ios/QingKeSchedule/Notifications/ReminderSettings.swift) | 停课与调课优先级、单双周组合、今日／周表／通知一致；原实现已通过 Sol 技术复审且视觉风格获用户确认；P3-07-R1 的顶部／间隙／底部显示与正式设置成功主路径已实现，但首轮复审发现首次设置／节次草稿冲突确认和写入失败收口缺口，修正与复审待完成；通知部分属 A08 未实施 |
