...
最新状态（2026-09-20）：P3-05／A03 周课表已实现、测试、通过 Sol 技术独立复审并经用户确认视觉通过（实现提交 `e3321bd`，验收收口 `5fb0bd2`），A03 当前范围的验收门槛关闭。P3-06／A06 的 R6 已完成 Sol 技术复审并获用户视觉验收。P3-06-R7 已实施（`b61b02f`：删除节次影响已有课程时的一次性破坏性汇总确认与原子级联保存；设置页保存阻断与写入失败统一使用居中红色弹窗、顶部和底部保存入口共用反馈），并完成 R1（`c7fd877`：反序／非连续编号无法安全重映射改为不可继续的红色错误弹窗、封闭确认旁路）与 R2（`f0c0d8e`：稀疏编号评估不再按编号跨度遍历）；该任务已通过 Sol 技术复审与用户视觉验收。P3-04-R8 随后把课程编辑器“保存被错误阻止”的底部红卡改为同一居中红色弹窗；R1 修正面板外触摸穿透，R2 把全屏拦截层改为弹窗面板之后的独立 sibling 底层并恢复时间选择器与级联摘要的真实拖动。P3-04-R8／R1／R2 现已通过 Sol 技术复审和用户视觉／交互验收。P3-07／A07 原实现已通过技术复审，用户已确认视觉风格；P3-07-R1 首轮复审发现的两项缺口已由 `31b48d1` 修正并通过 [Sol 再复审](p3-07-r1-review.md)（JVM 132／设备 110、7 张截图证据），本轮红色警告框与提示文案仍待用户视觉／观感验收，A07 功能验收保持打开。用户随后确认 D03（精确提醒优先、精确不可用时降级为非精确并明确标记「可能延迟」、采用可撤销的 SCHEDULE_EXACT_ALARM 而非 USE_EXACT_ALARM）并授权实施 A08 第一批（纯规划器与 Android 平台基础设施），该批已实施（`c4cb5e7`）；Sol 复审七项意见的 R1 返修已实施并自测（JVM 215／设备 148），**R1 待 Sol 独立复审，用户验收未进行**；提醒设置界面、运行时权限申请流程与课表编辑后的自动重算未纳入本批，A08 整体仍未完成。A10／A11、整个 P3 与完整 App 仍未授权或未完成。
...
A06 R6 已通过技术复审与用户视觉验收，P3-06-R7、R1、R2 已完成，已通过 Sol 技术复审与用户视觉验收；A07 原实现通过技术复审且视觉风格通过，R1 首轮两项缺口已修正并通过再复审、用户对新增警告框与文案的验收待进行，
...
| A07 | 周末停课、指定日期停课、调课日按指定星期上课，午休设置 | [教学日历设置](../../ios/QingKeSchedule/Notifications/ReminderSettings.swift) | 停课与调课优先级、单双周组合、今日／周表／通知一致；原实现已通过 Sol 技术复审且视觉风格获用户确认；P3-07-R1 的顶部／间隙／底部显示、草稿／已存节次冲突确认和写入失败收口均已实现并通过 Sol 再复审，新增警告框与文案待用户验收；通知的规划与平台基础设施属 A08 第一批（已实施、R1 待复审），提醒设置界面仍未实施 |
| A08 | 提醒开关、预设与自定义提前时间；修改课表后更新、删除后取消 | [通知规划与协调](../../ios/QingKeSchedule/Notifications/NotificationScheduling.swift)、[提醒界面](../../ios/QingKeSchedule/Features/ReminderSettingsSection.swift) | 拒绝权限仍可使用课表；测试时间、内容、取消和真机投递；第一批（纯规划器与 Android 平台基础设施）已实施（`c4cb5e7`）且 R1 返修已实施并自测、待 Sol 复审；提醒设置 UI、运行时权限流程与编辑后重算未实施 |
