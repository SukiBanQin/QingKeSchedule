...
最新状态（2026-09-19）：P3-05／A03 周课表已实现、测试、通过 Sol 技术独立复审并经用户确认视觉通过（实现提交 `e3321bd`，验收收口 `5fb0bd2`），A03 当前范围的验收门槛关闭。P3-06／A06 的 R6 已完成 Sol 技术复审并获用户视觉验收，A06 当前范围的验收门槛关闭。P3-07／A07 教学日历已实现、测试并通过 [Sol 独立技术复审](p3-07-review.md)：纯规则位于 `domain/AcademicCalendarRules.kt`，`ScheduleViewModel` 经 `updatePreferences` 原子写入，首次设置与正式设置共享「03 教学日历」终端面板，午休仅影响周课表展示；用户视觉验收尚未进行。A08／A10／A11、整个 P3 与完整 App 仍未完成且未授权。
...
A06 当前范围已通过技术复审与用户视觉验收；A07 已通过技术复审、等待用户视觉验收，
但 A01—A11 尚未全部验收，A02、A07—A11、整个 P3 和完整 App 仍未完成验收；后续阶段未授权。
...
| A07 | 周末停课、指定日期停课、调课日按指定星期上课，午休设置 | [教学日历设置](../../ios/QingKeSchedule/Notifications/ReminderSettings.swift) | 停课与调课优先级、单双周组合、今日／周表／通知一致；已实现、测试并通过 Sol 技术复审：Android 在首次设置与正式设置共享「03 教学日历」面板，写入立即持久化并驱动今日、周表，用户视觉验收待进行，通知部分属 A08 未实施 |
