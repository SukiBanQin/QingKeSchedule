...
但 A01—A11 尚未全部验收，A02、A08—A11、整个 P3 和完整 App 仍未完成验收；P4／A10 已授权但尚未实施，A11 及其后续阶段仍未授权。
...
| A10 | JSON 导入先校验、预览、确认后整体替换；导出课表备份 | [数据传输](../../ios/QingKeSchedule/Transfer/ScheduleDataTransfer.swift)、[文件界面](../../ios/QingKeSchedule/Features/DataTransferSection.swift) | D01 已确认继续使用版本 1，只迁移学期与课程且保留本机偏好；覆盖有效／无效／未知版本／超大文件、取消、写入失败及双向导入；P4 已授权、尚未实施 |
