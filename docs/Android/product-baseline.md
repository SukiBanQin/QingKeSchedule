...
但 A01—A11 尚未全部验收，A02、A08、A09、A11、整个 P3 和完整 App 仍未完成验收；P4／A10（含 R1）已实现、测试、通过 Sol 独立技术复审并获用户验收；P3-09／A11 已实施并自测（[实施记录](p3-09-a11-appearance.md)），**待 Sol 独立复审和用户验收**；A11 之后的阶段仍未授权。
...
| A11 | 跟随系统、浅色、深色；保持品牌、颜色、信息层级和终端风格 | [CourseStyle](../../ios/QingKeSchedule/Features/CourseStyle.swift)、[应用状态](../../ios/QingKeSchedule/State/ScheduleAppState.swift) | [只读分析](p3-09-a11-appearance-analysis.md)确认既有三态持久化与主题消费；已实现正式设置页 `06 外观／DISPLAY`、`ScheduleViewModel.setAppearanceMode` 单一路径、实际生效文字与响应式三选项（[实施记录](p3-09-a11-appearance.md)）；已覆盖即时切换、SYSTEM 动态跟随、重启持久化、相同值／写入中门禁、失败保留旧模式、小屏／大字体、48dp 触控与读屏语义，并修正一处按系统夜版资源取图的品牌 Logo 问题。**已实施并自测，待 Sol 独立复审和用户验收** |
