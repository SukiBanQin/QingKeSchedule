...
最新状态（2026-09-20）：P3-05／A03、P3-06-R7（含 R1／R2）与 P3-04-R8（含 R1／R2）已通过 Sol 技术复审和相应用户验收。P3-07／A07 原实现及 R1 已通过技术复审，新增警告框与文案仍待用户验收。D03 已确认；A08 第一批实现为 `c4cb5e7`，R1 `f4be82f` 经 [Sol 再复审](p3-08-r1-review.md) 发现重提交部分失败时的注册表／活动集合不一致、`cancelAll` 活动集合失真及权限撤销证据断言错误，**R1 未通过**；R2 `30e6c19` 已修正上述问题并自测，随后通过 Sol 独立技术复审，**用户验收未进行**；A08 第二批已实施提醒设置页（「04 上课提醒」开关、预设与自定义提前量、通知权限／渠道／精确闹钟状态）、仅显式申请 POST_NOTIFICATIONS 的权限流程、偏好与课表写入后自动重算和关闭取消（[实施记录](p3-08-a08-reminders-batch2.md)），但 [Sol 独立复审](p3-08-a08-reminders-batch2-review.md) 发现权限／渠道恢复后只刷新状态、不重新安排提醒，R1 已补齐能力恢复入口（权限结果与系统设置返回时幂等 `reconcile`）与设备回归并自测、**待 Sol 再复审**。窗口兜底再排程未实施，A08 整体仍未完成；A10／A11、整个 P3 与完整 App 仍未授权或未完成。
...
| A07 | 周末停课、指定日期停课、调课日按指定星期上课，午休设置 | [教学日历设置](../../ios/QingKeSchedule/Notifications/ReminderSettings.swift) | 停课与调课优先级、单双周组合、今日／周表／通知一致；原实现已通过 Sol 技术复审且视觉风格获用户确认；P3-07-R1 的顶部／间隙／底部显示、草稿／已存节次冲突确认和写入失败收口均已实现并通过 Sol 再复审，新增警告框与文案待用户验收；通知规划与平台基础设施属 A08 第一批（含 R1／R2，已通过 Sol 独立技术复审），提醒设置界面属 A08 第二批（R0 复审未通过、R1 已实施并自测、待 Sol 再复审） |
| A08 | 提醒开关、预设与自定义提前时间；修改课表后更新、删除后取消 | [通知规划与协调](../../ios/QingKeSchedule/Notifications/NotificationScheduling.swift)、[提醒界面](../../ios/QingKeSchedule/Features/ReminderSettingsSection.swift) | 拒绝权限仍可使用课表；测试时间、内容、取消和真机投递；第一批（含 R1／R2）已通过 Sol 独立技术复审；第二批已实施提醒设置 UI、运行时权限流程与提交后自动重算，R1 补齐能力恢复（权限授予／系统设置返回后重新登记），待 Sol 再复审；用户验收未进行，窗口兜底再排程未实施 |
