...
> 后续状态：提醒设置界面、运行时通知权限申请流程与课表编辑／保存后的自动重算已由 A08 第二批实施，见
> [第二批实施记录](p3-08-a08-reminders-batch2.md)；超出滚动窗口的兜底再排程仍未实施。

第一批当时未纳入：提醒设置界面、运行时通知权限申请流程、课表编辑／保存后的自动重算、超出滚动窗口的再排程
策略。前三项已由第二批实施并在 R1 后通过 Sol 独立技术复审；窗口兜底仍未实施。
...
[P3-08-R1／R2 独立技术复审](p3-08-r1-review.md)。A08 第二批已实施设置 UI、运行时权限流程与编辑后自动
重算，并在 R1 后通过 Sol 独立技术复审；窗口兜底、A10／A11、整个 P3 与完整 App 仍未完成、未授权。
