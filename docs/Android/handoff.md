...
P1-03 从 `ec236b9` 开始执行，授权提交为 `274f3b9`，实施提交为 `7d34c78`。专项复审确认 API 37／AGP 9.4／Gradle 9.6／built-in Kotlin 改动及主机侧验证通过；未发现应用代码缺陷。P1-03-R2 提交 `2e5b8ec` 已修正 Android 17 的项目源码／现有依赖边界，并如实记录单次 API 37 AVD 启动未完成。独立专项复审接受 R2 的文档修正和失败证据，但复审环境在禁快照、无窗口、软件图形下仍持续 `offline` 并出现 HVF／`mprotect` 错误，未取得 `sys.boot_completed=1`。下一步只做 P1-03-R3 设备环境恢复与运行验证；P1-03、P1 和用户验收均未通过，不进入 P2。详见 [P1-03 专项复审](p1-03-review.md)和 [R2 独立启动诊断](evidence/p1-03-r2-review-api37-attempt-20260908.txt)。
