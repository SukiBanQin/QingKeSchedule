...
当前结论：**Android 分支上的 P1 授权范围和审查门槛已具备关闭条件；P1 阶段整体暂不
标记关闭，等待将 P1-04 修正同步到 `IOS` 并重跑 iOS 测试。** 这不是用户验收，也不
授权进入 P2。同步任务完成后仍需核对 `IOS` 提交范围、测试结果和两分支一致性，再记录
P1 阶段关闭。
...
更新时间：2026-09-08。P1-01、P1-03、P1-04 均已通过独立复审；API 37 模拟器环境阻塞已解除。当前仅等待将 P1-04 修正同步到 `IOS` 并复测，之后再记录 P1 阶段关闭；不进入 P2，用户未验收。详见 [P1-04 任务记录](p1-04-unknown-fields.md)。
...
- 用户已确认版本 1 未知字段两端严格拒绝；P1-04 已在 `Android` 分支通过独立专项复审。下一步是将同一 iOS 修正同步到 `IOS` 分支并重跑 iOS 测试，建议执行窗口使用 Terra／中；完成后分析窗口再核对 P1 阶段关闭。实际窗口模型和档位未核实，不要替用户变更设置。
...
- 修正提交 `3924d26`、`985cdd6`、P1-03-R4 及 P1-04 提交 `4b7ff3e` 均已通过独立复审。版本 1 未知字段策略已决定为两端严格拒绝；P1-04 尚待同步到 `IOS` 并复测，P1 尚未关闭，不进入 P2。
