...
当前结论：**P1 授权范围和审查门槛已完成；P1 尚未关闭，等待用户验收及是否安排后续阶段的明确授权。**
本核对不代表完整 App 已完成，不代表 A01—A11 已全部验收，也不授权自动进入 P2。D01、D03、
正式发行范围等未决事项继续保留；如用户授权进入后续阶段，应另立任务并明确范围。

详细的 IOS 同步复审见 [P1-04-IOS-SYNC 复审](p1-04-ios-sync-review.md)。

## P1-04 未知字段复审状态（最新，2026-09-08）
...
更新时间：2026-09-08。P1-01、P1-03、P1-04 及 P1-04-IOS-SYNC 均已通过独立复审；API 37 模拟器环境阻塞已解除，两个开发分支已同步。当前等待用户验收及后续阶段授权；不进入 P2。详见 [P1-04 任务记录](p1-04-unknown-fields.md) 和 [IOS 同步复审](p1-04-ios-sync-review.md)。
...
- 用户已确认版本 1 未知字段两端严格拒绝；P1-04 已在 `Android` 分支通过独立专项复审，P1-04-IOS-SYNC 已在 `IOS` 分支同步并通过复审。下一步等待用户验收和后续阶段授权；不自动进入 P2。实际窗口模型和档位未核实，不要替用户变更设置。
...
- 修正提交 `3924d26`、`985cdd6`、P1-03-R4、P1-04 提交 `4b7ff3e` 及 IOS 同步提交 `0a3252d` 均已通过独立复审。版本 1 未知字段策略已决定为两端严格拒绝；P1 授权范围审查已完成，P1 尚未关闭（等待用户验收），不进入 P2。
...
未知字段，P1-04 已通过独立专项复审；下一步由执行窗口将 P1-04 的 iOS 修正同步到 `IOS`
并重跑 iOS 测试，完成后再单独核对 P1 阶段是否关闭。本轮没有 P2 授权，不自动派发或实施 P2。
