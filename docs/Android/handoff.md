...
- 根 README、该文档测试、本文及 `ios/` 已提交为 `89a4a2809702a8748e9d51f4604aeaed8e464712` 并推送 `origin/Android`；远端与本地一致，未跟踪 `.vscode/` 保留。`origin/main` 当前为 `8a379403e4a4131ed7d2e75553f8072075c3f2ad`；用户已授权非强推合并，但本次主审明确要求先审阅全量测试阻断证据，故 **尚未合并／推送 main**。当前准确状态是 **iOS 源码已同步至 Android 工作树、Mac 本机签名说明和文档验证完成；全量 UI 测试未通过（模拟器启动阻断），iOS 未经独立审查／实体设备验收，也没有公开 iOS 版本**。
...
本轮起点 `6f51aa4`，分支 `Android`；已有未跟踪 `.vscode/` 属于用户现场，本轮保留且不纳入提交。仅修改流程文档与对应验证，不改应用代码或客户端配置。下方 A08 按已观察范围验收及 P6／D04 未完成的状态保持，不启动后续开发。本次提交和验证结果见交付消息。

## P6／A08 用户按已观察范围验收通过（最新，2026-09-23）

用户自行再次测试小米 10，确认清理后台后青课仍发出通知，伴随震动，并在锁屏显示；随后明确表示“通过吧”。结合此前真实权限弹窗、重启恢复、强制深度 Doze、非精确降级及小米后台策略的设备证据，记录 **A08 用户按当前已观察范围验收通过**。成立的厂商条件仍是青课“自启动”开启、省电策略设为“无限制”、提醒类别允许悬浮／震动／锁屏完整显示；在小米默认推荐省电策略下曾实际发生后台闹钟滞留，不能抹去。

2026-09-23 07:30 的自然长时待机测试因手机早晨没电而没有可判定结果；用户决定不再重测。真实系统 UI 手动拒绝权限后的完整继续使用路径、非精确能力下长期待机，以及其他厂商设备仍未验证，作为限制保留。本次用户称测试课程已自行清理；手机目前未连接 Mac，无法独立核对 `A08OVERNIGHT` 是否删除、当前第 1 节时间及系统闹钟。用户明确表示**不需要把第 1 节设回 18:50**，此前“待连接后恢复”的计划作废；不得再自动改动手机节次。

准确状态：A08 三批应用实现已通过 Sol 技术复审，现获用户对真机实际提醒体验的验收；**P6 最终全量回归、A01—A11 证据收口及 D04 范围决定仍未完成**，不能把 A08 验收等同于 P6／完整 App 发布。下一步建议在 `Android` 分支进行 P6 最终全量构建／测试、设备与文档证据索引及已知限制核对，再请用户确认 P6 阶段结果；D04 另行决定，不发布、不合并 `main`。

## P6／A08 自然待机测试取消，待清理设备（历史，2026-09-23）

> 后续状态：用户已自行清理测试课程，并决定不恢复第 1 节到 18:50；设备未连接，清理结果未独立核对。以本文首节为准。

用户反馈早晨起床时小米 10 已没电，未提供 07:30 的通知、悬浮或震动观察；用户判断无需再次测试该项目。因此 2026-09-23 自然长时待机测试**无结果，不标通过或失败**，不再安排重测。07:35 的一次性回访已删除。当前 `adb devices -l` 只识别到 `emulator-5554`，未识别小米 10，无法读取早晨系统日志，也无法在设备上执行收尾。

待手机充电并重新连接 Mac 后，按先前授权只删除测试课 `A08OVERNIGHT`，把临时第 1 节 **07:30–07:35** 恢复为 **18:50–18:55**，核对提醒闹钟；保留用户 `test` 课程和第 6 节。完成清理前不能称设备数据已恢复。之后可整理 A08 已取得的真机证据与明确未验证项，再进行 P6 最终回归和用户范围确认；不凭“感觉没问题”自动将 A08／P6 记作验收通过。

## P6／A08 小米 10 旧提醒核对与自然待机布置（最新，2026-09-22 21:28）

用户最终确认 19:05／19:20 未看到额外的青课通知，只在 19:35 收到 `A08MOVE`；手机系统事件日志在 19:00–19:39 也仅有 19:35:01 的一条青课通知入队。因此本次旧时间未发现重复投递，但用户表示可能未持续留意，人工观察不能写成逐秒确认。此前记录的旧时间“待观察”以本节为准。详见 [小米 10 真机阶段证据](evidence/p6-a08-mi10/README.md)。

经用户同意，已通过应用正式编辑器清理仅本轮新增的 `A08LOCK`／`A08MOVE`，并先将第 1 节恢复为 18:50–18:55；用户的 `test` 课程、第 6 节及其 2026-09-28 22:19 精确闹钟未动。随后用户同意安排自然长时待机测试：暂把第 1 节设为 **07:30–07:35**，新增 `A08OVERNIGHT`（2026-09-23 星期三，仅第 1 周、第 1 节，教室 `MI10`）。21:28 核对系统已登记 2026-09-23 07:30:00 的精确 `RTC_WAKEUP`（`window=0`、`exactAllowReason=permission`）；设备当时仍为 AC 充电、93% 电量，尚未证明自然待机投递。已请用户拔掉充电线与 Mac USB、锁屏静置到明早 07:30，并约定明早核对通知、悬浮与震动；本线程安排 07:35 回访。明早核对后需删除 `A08OVERNIGHT`、恢复第 1 节 18:50–18:55，保留 `test` 与第 6 节，再核对闹钟。

本轮仅操作测试设备并维护文档，不修改应用代码。**A08／P6 尚未关闭；自然长时待机结果和用户整体验收均未完成。** 不发布、不合并 `main`。

## P6／A08 小米 10 下午加速验证（最新，2026-09-22）

用户确认小米 10 是备用机并要求立即完成可加速的时间验证。本轮只操作设备和维护文档，未改应用代码；开始基准为 `Android` 分支 `3cb35c1bfd50aa6cb38ca6196a6df91aa4f18b83`（与 `origin/Android` 一致），无关的未跟踪 `.vscode/` 保留不动。完整操作、时间线与截图见 [小米 10 真机阶段证据](evidence/p6-a08-mi10/README.md)。

- 关闭／重新开启提醒后，系统中的课程闹钟分别撤销／恢复；正式编辑器删除 `A08CANCEL` 后其闹钟撤销，修改 `A08MOVE` 从第 3 节到第 4 节后旧闹钟撤销、新闹钟出现。上述旧时间尚未逐一到点观察，不能把“闹钟已撤销”写成“实际未投递”已验收。
- 默认小米“智能限制后台运行（推荐）”策略下，青课虽已开启自启动、获通知和精确闹钟权限，清最近任务＋锁屏后的 14:25 精确闹钟仍滞留 `Pending user blocked background alarms`，未通知。这是本机真实厂商后台限制。只把青课改为“无限制”后，16:12 再次清最近任务＋锁屏的精确提醒成功投递，锁屏完整显示课程名、时间和教室；用户亲自确认看到了且有震动。小米“自启动＋青课无限制”是此次成功的设备条件，不外推至默认策略或其他厂商。
- 使用 `dumpsys battery unplug`／`dumpsys deviceidle force-idle` 验证真实 `IDLE` 深度 Doze，屏幕处于 `Dozing`；16:25 精确提醒实际投递。之后已执行 `unforce`／`battery reset`，深／浅空闲均恢复 `ACTIVE`、AC 充电恢复。此项是强制 Doze，不等同自然长时休眠。
- 临时将精确闹钟 app-op 设为 `ignore`。青课处于小米“无限制”时仍被白名单允许精确闹钟，不能据此测试降级；临时改回推荐策略并在前台重存后，系统登记非精确时间窗口，设置页显示“不可用／部分可能延迟”，16:40 名义提醒约 16:46 才投递，通知含“（可能延迟）”。测试后 app-op 恢复 `default`、青课省电策略恢复“无限制”；设置页显示“精确闹钟：可用”，两条未来课程闹钟均恢复 `window=0` 精确模式。
- 原定 2026-09-22 18:35 的旧自动回访已删除，避免重复打断。用户自己新增的 `test` 课程与第 6 节时间未改。仅针对本轮新增的 `A08LOCK`／`A08MOVE` 课程及临时改动的第 1 节，已向用户询问是否清理／恢复，回复前不自行删除或覆盖。
- 尚未关闭：自然长时休眠与非精确能力下的深度 Doze／长期后台投递；旧时间不再投递的逐一到点观察；真实系统 UI 手动拒绝权限后继续使用。**A08 仍是部分真机验证，未经用户整体验收；P6 未关闭。** 不实施发布、不合并 `main`。

## P6／A08 小米 10 真机验证进行中（历史，2026-09-21）
