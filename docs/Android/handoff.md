...
P1-04 已从基准 `cb7323f` 实施：iOS 导入边界在
...
[P1-04 任务记录](p1-04-unknown-fields.md)。本任务涉及跨端协议，当前状态为实施完成、
待独立复审；未修改共享 schema、Android 行为、重复 ID、节次顺序、D01／D03 或 UI，
不进入 P2。复审通过后再单独核对 P1 是否关闭，并安排同步到 `IOS` 分支。
