...
- 用户已确认采用实施计划中的模型档位建议。P1-01-R2 已复审通过；当前下一项是 Terra／高执行 P1-03 工具链升级与 API 37 验证。选择高档是因为 AGP 9 built-in Kotlin 会联动 Kotlin serialization、Compose compiler plugin、Gradle DSL 和设备验证。未知字段策略继续等待产品决定；P1-03 不修改该行为、不进入 P2。实际窗口模型和档位未核实，不要替用户变更模型、档位或服务商配置。
...
P1-01-R2 已复审通过，不再派发修正。用户已授权 P1-03 工具链升级验证，下一步由 Terra／高基于当前 `Android` 分支实施；范围和验收条件如下。不进入 P2，完成提交也必须由分析审查窗口复审，不能直接标记 P1 完成。
...
- 允许修改 Android 工具链和构建文件、Wrapper、`Android/README.md`，以及为验证真实工具链回归所必需的 Android 测试。主应用 Kotlin 源码只有在工具链编译迁移确实要求时才能最小修改，并须单独说明原因和行为不变证据。
...
- 若 API 37 system image、AGP/Kotlin 插件或设备环境不可用，保留最小错误证据并回传，不降级目标、不把 API 35 历史证据冒充本轮通过。只提交本任务改动并推送 `Android`；Terra 回传提交范围、完整命令、测试数量、设备信息、限制和复审请求。
