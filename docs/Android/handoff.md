...
P1-03 从 `ec236b9` 开始执行，授权提交为 `274f3b9`，实施提交为 `7d34c78`。专项复审确认 API 37／AGP 9.4／Gradle 9.6／built-in Kotlin 改动及主机侧验证通过；未发现应用代码缺陷。P1-03-R2 提交 `2e5b8ec` 已修正 Android 17 的项目源码／现有依赖边界，并如实记录单次 API 37 AVD 启动未完成。独立专项复审接受 R2 的文档修正和失败证据。P1-03-R3 已在持久 SDK 和全新 ARM64 AVD 上按默认与禁快照/软件图形各尝试一次，但仍未取得 `sys.boot_completed=1`；软件图形对照仍出现 HVF／`mprotect` 错误。下一步需改用 API 37 ARM64 真机或另一可用宿主，不能以 API 35 代替；P1-03、P1 和用户验收均未通过，不进入 P2。详见 [P1-03 专项复审](p1-03-review.md)和 [R3 持久 SDK 诊断](evidence/p1-03-r3-api37-persistent-sdk-attempt-20260908.txt)。
...
- 修正提交 `3924d26` 已通过 R1/R2/R3 复审，数字词法修正提交 `985cdd6` 也已独立复审通过；P1-01 的已授权基础范围现已通过。P1-02 提交 `e80ea2f` 的文档、APK 预检及工具链记录已复审通过；P1-02-R1 提交 `17d4357` 的 ARM64 探测记录及 P1-02-R2 提交 `23e0501` 的安装启动证据也已复审通过。未知字段策略仍未决定；P1-03-R2 的文档边界和失败记录有效，API 37 设备门槛仍待 P1-03-R3，不能进入 P2。
...
下一步只执行 P1-03-R3。建议执行窗口使用 Terra／中：应用和文档缺陷已经排除，剩余工作是恢复一个能完成开机的 API 37 ARM64 或等效设备并补齐运行证据。只使用官方 Emulator 启动器或 Android Studio Device Manager；优先在持久 SDK 中重装稳定 Emulator／API 37 image 并新建无旧快照 AVD，仍失败则换等效设备。取得 `sys.boot_completed=1` 后才安装 APK，完成两次冷启动、Activity／进程、截图和清空后无崩溃 logcat。完整要求见 [P1-03 专项复审](p1-03-review.md)。完成后仍须复审，不进入 P2，不标记 P1 或用户验收完成。
