...
## P1-03-R4 最新环境恢复状态（优先于下方历史，2026-09-08）
...
**运行证据已补齐，待独立复审；P1 仍未完成，用户未验收，不进入 P2。**
详见 [R4 修复和运行记录](p1-03-validation.md) 的首节；下方旧设备阻塞和换宿主建议
仅作历史。未知字段策略、D01/D03 和正式发行决定不变。本轮构建 70 项 up-to-date，
读取既有双变体各 20 项报告，不冒充重新执行 JVM 测试。
系统开发者模式按用户授权已开启，但单独开启未解决问题；未证明其必要性。
预览版仅隔离保留，实际成功使用稳定版，未修改项目依赖。
下一步仅独立核对本轮配置修复及 API 37 运行证据，无需 Terra 继续排查环境或实施 P2。
本次文档提交和推送结果从交付消息或 git log 查询，不自引用未生成提交。
...
P1-03 从 `ec236b9` 开始执行，授权提交为 `274f3b9`，实施提交为 `7d34c78`。专项复审确认 API 37／AGP 9.4／Gradle 9.6／built-in Kotlin 改动及主机侧验证通过；未发现应用代码缺陷。P1-03-R2 提交 `2e5b8ec` 已修正 Android 17 的项目源码／现有依赖边界，并如实记录单次 API 37 AVD 启动未完成。独立专项复审接受 R2 的文档修正和失败证据。P1-03-R3 提交 `23743be` 的持久 SDK／干净 ARM64 AVD 失败证据已专项复审通过，但仍未取得 `sys.boot_completed=1`；软件图形对照仍出现 HVF／`mprotect` 错误。下一步需改用 API 37 ARM64 真机或另一可用宿主，不能以 API 35 代替；P1-03、P1 和用户验收均未通过，不进入 P2。详见 [P1-03 专项复审](p1-03-review.md)和 [R3 持久 SDK 诊断](evidence/p1-03-r3-api37-persistent-sdk-attempt-20260908.txt)。
...
更新时间：2026-09-08。P1-01-R2 已复审通过，提交为 `985cdd6`。P1-03 提交 `7d34c78` 的工具链和主机侧验证已通过专项复审；P1-03-R2 提交 `2e5b8ec` 的源码／依赖层说明和设备失败边界也已通过专项复审。用户截图与独立诊断继续显示当前 API 37 AVD 受 HVF／`mprotect` 环境问题阻塞，设备门槛仍未满足。下一步仅为 P1-03-R3；未知字段策略继续待决定，P1 仍未完成、不进入 P2，用户未验收。本轮没有修改应用代码、iOS、Web 或共享协议，也未启动子 Agent。详见 [P1-03 验证记录](p1-03-validation.md)和 [P1-03 专项复审](p1-03-review.md)。
...
- D02：已确认包名 `com.qingke.schedule`、最低 API 26。P1-03 已采用 API 37.0 + AGP 9.4.0 + Gradle 9.6.0 + Build Tools 36.0.0 + JDK 17；保持 minSdk 26，工具链和主机侧验证已通过专项复审。P1-03-R2 已修正 Android 17 依赖层表述并复验 Debug APK/元数据，但 API 37 设备仍未完成开机。API 37.1／37.2 没有本轮核实的完整官方兼容映射，不纳入此次升级。
- D03：精确提醒不可用时的降级方式与文案尚未确定。
- D04：已确认 P1 仅做个人安装验证和可复现 debug 构建，不配置商店签名；正式发布范围仍未确定。
- 用户已确认采用实施计划中的模型档位建议。P1-03-R2 已专项复审；下一步由执行窗口用 Terra／中完成 P1-03-R3，恢复可用 API 37 设备环境并补齐运行证据。任务边界明确，模型无法绕过宿主虚拟化限制；同一 Mac 仍失败时应换等效 API 37 设备。未知字段策略继续等待产品决定；P1-03-R3 不修改该行为、不进入 P2。实际窗口模型和档位未核实，不要替用户变更模型、档位或服务商配置。
...
- 修正提交 `3924d26` 已通过 R1/R2/R3 复审，数字词法修正提交 `985cdd6` 也已独立复审通过；P1-01 的已授权基础范围现已通过。P1-02 提交 `e80ea2f` 的文档、APK 预检及工具链记录已复审通过；P1-02-R1 提交 `17d4357` 的 ARM64 探测记录及 P1-02-R2 提交 `23e0501` 的安装启动证据也已复审通过。未知字段策略仍未决定；P1-03-R2 的文档边界和失败记录有效，P1-03-R3 提交 `23743be` 的持久 SDK／干净 ARM64 AVD 失败证据记录已复审通过，但 API 37 设备门槛仍未满足，不能进入 P2。
...
P1-03-R3 失败证据已复审通过。下一步只使用 API 37 ARM64 真机或另一可正常开机的 API 37 宿主补齐运行证据；取得 `sys.boot_completed=1` 后才安装 APK，完成两次冷启动、Activity／进程、截图和清空后无崩溃 logcat。完整要求见 [P1-03 专项复审](p1-03-review.md)。完成后仍须复审，不进入 P2，不标记 P1 或用户验收完成。
