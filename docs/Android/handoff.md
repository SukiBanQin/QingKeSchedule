...
- `CourseColorDialog` 默认“网格”，新增等宽且 48dp 的“网格／光谱／滑块”入口；三页均以既有 ViewModel `editor.color` 为唯一颜色源。网格提供灰阶和 30+ 主色相／明暗单元；光谱为确定的 HSV 饱和度／明度平面加色相拖动条和选择指示器；滑块为 RGB 点击／拖动轨道并保留 `HEX / ADVANCED` 辅助输入。存储和 editor 仍只接收严格 `#RRGGBB`。
