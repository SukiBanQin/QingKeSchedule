...
3. 首轮证据为程序化核对（Compose 断言、节点 bounds、像素分类）；后续 R2 另以真实系统输入验证面板内部
   滑动，用户最终已确认视觉与真实交互通过。
...
首轮 Sol 复审未通过；R1 修正（JVM 169、设备 128）解决遮罩穿透，但用户真机交互发现其同时拦截弹窗
内部拖动；R2 以 sibling 遮罩结构完成修正（JVM 169、设备 133、lint 0 errors、5 张新截图＋adb 真实输入
对照），现已通过 Sol 技术复审，并经用户确认视觉与真实交互通过。该验收只关闭 P3-04-R8／R1／R2，
不扩大为 P3-07-R1、整个 P3 或完整 App 验收。P3-06-R7（含 R1／R2）已通过 Sol 技术复审与用户视觉
验收。下一项仅授权 P3-08／A08 的只读分析；A08 实施、A10／A11 仍未授权。
